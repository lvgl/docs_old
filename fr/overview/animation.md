...
Vous pouvez faire évoluer automatiquement la valeur d'une variable entre une valeur initiale et une valeur finale en utilisant les animations.
...
Par conséquent, `lv_anim_create()` supprimera les animations variable/fonction déjà existantes.
...
- **lv_anim_path_bounce** rebondit un peu sur la valeur finale (comme en frappant un mur)
...
La fonction `lv_anim_speed_to_time(speed, start, end)` calcule la durée requise en millisecondes pour atteindre la valeur finale à partir de la valeur initiale avec une vitesse donnée. 
